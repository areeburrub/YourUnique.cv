export const notes = `# Executive Serif template notes
...
This layout **requires** top-level \`location\`. Optional \`headline\` and education \`gpa\` are rendered when present. Do not send certifications, languages, or awards — they are not rendered.

Write for a senior / leadership reader: title first, then company. Prefer scope, outcomes, and teams over implementation detail.

## Layout (how this page reads)

- Centered name, gold rule, optional headline, then contact
- Experience rows lead with **title**, then company / location / dates
- Dates render as one \`dates\` string: \`Mar 2024 – Present\` (en dash). Do not send startDate/endDate or date objects.
- Projects are labeled Selected Work. Omit them when the profile is stronger as a leadership page.
...
- \`headline\`: a real senior title from the profile or JD. Omit if none.
...
Treat as selected work, not a student project dump.

- Put the live site in \`url\` (host/path). Extra GitHub / Product Hunt links go in \`links\`.
- Include \`stack\` when known.
- Bullets should focus on outcome and scope.
