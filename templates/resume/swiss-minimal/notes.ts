export const notes = `# Swiss Minimal template notes
...
This layout **requires** top-level \`location\`. Optional \`headline\` renders under the name. Do not send GPA, certifications, languages, or awards — they are not rendered.

## Layout (how this page reads)

- Large left-aligned name, optional headline, then a quiet contact row
- Wide-tracked section labels with a hairline rule
- Experience: title and dates on one row, company and location on the next
- Dates render as one \`dates\` string: \`Mar 2024 – Present\` (en dash). Do not send startDate/endDate or date objects.
- Projects render as a title, muted stack, then bullets
