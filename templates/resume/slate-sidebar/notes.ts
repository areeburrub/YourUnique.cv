export const notes = `# Slate Sidebar template notes
...
This layout **requires** top-level \`location\`. Optional \`headline\` and education \`gpa\` are rendered when present. Skills live in the left sidebar. Do not send certifications, languages, or awards — they are not rendered.

## Layout (how this page reads)

- Dark left rail: name, optional headline, contact, then skills
...
- Keep skill categories short — the sidebar is narrow. Prefer 4–6 compact groups.
...
- \`skills[].items\`: one comma-separated string. Keep each group tight so it fits the rail.
