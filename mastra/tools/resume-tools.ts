...
export const createResumeTool = createTool({
	id: "create_resume",
	description:
		"Create a resume from structured document JSON matching the selected template's inputSchema, and queue the PDF. Never send HTML, Typst, or LaTeX. Inline **bold** and *italic* in string fields is OK. One resume per turn — a second call updates the first. When tailored to a job, always pass companyName, roleTitle, and jobLink when known.",
	inputSchema: z.object({
		name: z
			.string()
			.min(1)
...
export const updateResumeDocumentTool = createTool({
	...
	description:
		"Replace the structured resume JSON for an existing generation and queue a new PDF. Document must match that resume's template inputSchema. Send the full updated document. Inline **bold** and *italic* in string fields is OK; do not send HTML.",
	inputSchema: z.object({
		id: z.string().min(1),
		document: z.record(z.string(), z.unknown()),
	}),
