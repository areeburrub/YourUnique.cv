...
- Ask for the company name and job title only if missing from the message
...
YourUnique.cv output is a **structured JSON document** via `create_resume` / `update_resume_document` matching the selected template's `inputSchema` from `get_resume_template_notes`. Do not write Typst, LaTeX, HTML, or a Markdown resume document. Prose fields may use inline `**bold**`, `*italic*`, and `[label](https://url)` on a few key terms.
