...
| PDF | **HTML templates + Playwright** (Trigger.dev) | Agent fills per-template JSON Schema (`source_json`); app renders built-in React or custom Handlebars HTML to A4 PDF |
...
- **No Inngest** — Trigger.dev wins on price (Free → Hobby $10 vs Inngest Pro $99) and on running Playwright/Chromium outside Vercel serverless limits.
...
4. Trigger.dev: HTML template compile → PDF download
...
- Browser-only PDF as the only compile path (optional for preview later)
